    ...
    return new Promise((r, j) => {
        fs.readFile(filePath, 'utf8', function (err, files) {
            ...
            while(whileRet = result.match(/import(\s+)(\w+)(\s+)from(\s+)["|'](.\/components|..\/components|.)\/(\w+)(.vue)["|']/)){
                ...
                config[whileRet[2]] = '~@/components/' + whileRet[2] + '.wpy';
                result = result.replace(new RegExp(self.fromCamelbak(whileRet[2]),'g'),whileRet[2]);
            }
            //window. 2 global.
            ...
            fs.outputFileSync(filePath.replace('.vue','.wpy'), result);
            if (err) {
                j(err);
            } else {
                // console.warn('handlePx2Rem:' + filePath + ' toStr');
...
Multipage.prototype.apply = function (compiler) {
    ...
    compiler.plugin("emit", function (compilation, callback) {
        ...
        self.walk('../../src',(file) => {
            console.log(file);
            self.handleReplace(file).then(() => {
                return self.handlePx2Rpx(file);
            }).then(() => {
                return self.handleReplaceCss(file);
            }).then(() => {
                callback();
            })
        })
